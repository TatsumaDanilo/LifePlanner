...
const AddHabitForm: React.FC<Props> = ({ type, initialData, onBack, onSave }) => {
  ...
  useEffect(() => {
    if (initialData) {
        ...
        if (initialData.timerDefault) setTimerDefault(initialData.timerDefault);
        if (initialData.timerDuration) setCountdownDuration(initialData.timerDuration);
              ...
              {list.map(item => (
                  <div key={item.id}>
                      <div className="flex items-center gap-2 bg-zinc-900 border border-white/5 rounded-xl p-3">
                          ...
                          <button type="button" onClick={() => setActiveParentId(item.id)} className="p-1.5 rounded-lg bg-white/5 text-zinc-400 hover:text-white"><Plus size={14} /></button>
                          <button type="button" onClick={() => deleteMicroHabit(item.id)} className="p-1.5 rounded-lg bg-white/5 text-red-400/70 hover:text-red-400"><Trash2 size={14} /></button>
                      </div>
      ...
      {type === 'value' && (
          <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                  ...
                  <button type="button" onClick={toggleTargetWeight} className={`w-12 h-7 rounded-full transition-colors duration-300 relative ${targetWeightEnabled ? 'bg-blue-500' : 'bg-zinc-700'}`}><div className={`absolute top-1 bottom-1 w-5 h-5 bg-white rounded-full shadow-md transition-all duration-300 ${targetWeightEnabled ? 'left-6' : 'left-1'}`} /></button>
              </div>
              <div className={`transition-opacity duration-300 ${targetWeightEnabled ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
                  <div className="flex gap-3 mb-4">
                      <div className="flex-1 h-14 bg-zinc-900 border border-white/10 rounded-[20px] px-2 flex items-center justify-between">
                          <button type="button" onClick={() => adjustWeight(-0.1)} className="w-10 h-10 flex items-center justify-center text-zinc-500 active:bg-white/5 rounded-full active:text-white transition-colors"><Minus size={18} /></button>
                          ...
                          <button type="button" onClick={() => adjustWeight(0.1)} className="w-10 h-10 flex items-center justify-center text-zinc-500 active:bg-white/5 rounded-full active:text-white transition-colors"><Plus size={18} /></button>
                      </div>
                      <button type="button" onClick={toggleWeightUnit} className="w-20 h-14 bg-zinc-900 border border-white/10 rounded-[20px] flex items-center justify-center text-sm font-bold text-zinc-300 active:bg-white/5 transition-colors">{weightUnit === 'kg' ? 'KG' : 'LBS'}</button>
                  </div>
                  <div className="flex bg-zinc-900 p-1 rounded-[16px] border border-white/5">
                      <button type="button" onClick={() => setGoalDirection('lose')} className={`flex-1 py-2.5 rounded-[12px] text-[10px] font-black uppercase tracking-wider transition-all ${goalDirection === 'lose' ? 'bg-white text-black shadow-lg' : 'text-zinc-500'}`}>Lose</button>
                      <button type="button" onClick={() => setGoalDirection('maintain')} className={`flex-1 py-2.5 rounded-[12px] text-[10px] font-black uppercase tracking-wider transition-all ${goalDirection === 'maintain' ? 'bg-white text-black shadow-lg' : 'text-zinc-500'}`}>Maintain</button>
                      <button type="button" onClick={() => setGoalDirection('gain')} className={`flex-1 py-2.5 rounded-[12px] text-[10px] font-black uppercase tracking-wider transition-all ${goalDirection === 'gain' ? 'bg-white text-black shadow-lg' : 'text-zinc-500'}`}>Gain</button>
                  </div>
              </div>
                          ...
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pt-2">
                              <div className="flex gap-2 flex-wrap justify-end">
                                  {WEEKDAYS.map((day, idx) => (
                                      <button key={day} type="button" onClick={() => toggleDaySelection(idx)} className={`w-9 h-9 rounded-full flex items-center justify-center text-[10px] font-black uppercase transition-all ${selectedDays.includes(idx) ? 'bg-white text-black shadow-lg scale-105' : 'bg-zinc-800 text-zinc-500 border border-white/5'}`}>{day.charAt(0)}</button>
                                  ))}
                              </div>
                          </motion.div>
                      )}
                      {frequencyType === 'days_per' && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pt-2 flex justify-end">
                               <div className="h-12 bg-zinc-900 border border-white/10 rounded-[16px] px-2 flex items-center justify-between w-40">
                                  <button type="button" onClick={() => setDaysPerWeek(Math.max(1, daysPerWeek - 1))} className="w-8 h-8 flex items-center justify-center text-zinc-500 active:bg-white/5 rounded-full active:text-white transition-colors"><Minus size={16} /></button>
                                  <span className="text-sm font-black text-white">{daysPerWeek} days</span>
                                  <button type="button" onClick={() => setDaysPerWeek(Math.min(7, daysPerWeek + 1))} className="w-8 h-8 flex items-center justify-center text-zinc-500 active:bg-white/5 rounded-full active:text-white transition-colors"><Plus size={16} /></button>
                               </div>
                          </motion.div>
                      )}
                  </AnimatePresence>
              ...
              <div className="mb-6">
                  ...
                  <div className="flex items-center justify-center gap-3">
                       <div className="h-14 bg-zinc-900 border border-white/10 rounded-[20px] px-2 flex items-center justify-between min-w-[140px]">
                          <button type="button" onClick={() => setCountValue(Math.max(1, countValue - 1))} className="w-10 h-10 flex items-center justify-center text-zinc-500 active:bg-white/5 rounded-full active:text-white transition-colors"><Minus size={18} /></button>
                          <span className="text-xl font-black text-white w-8 text-center">{countValue}</span>
                          <button type="button" onClick={() => setCountValue(countValue + 1)} className="w-10 h-10 flex items-center justify-center text-zinc-500 active:bg-white/5 rounded-full active:text-white transition-colors"><Plus size={18} /></button>
                       </div>
                       <div className="relative">
                          <button type="button" onClick={() => setIsUnitPickerOpen(!isUnitPickerOpen)} className="h-14 bg-zinc-900 border border-white/10 rounded-[20px] px-4 flex items-center justify-between gap-2 min-w-[100px] active:bg-white/5 transition-colors">
                          ...
                          <AnimatePresence>
                              {isUnitPickerOpen && (
                                  <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} className="absolute top-full mt-2 left-0 w-full min-w-[120px] bg-zinc-800 border border-white/10 rounded-[16px] overflow-hidden shadow-xl z-50">
                                      <div className="py-1">
                                          {HABIT_UNITS.map(u => (<button key={u} type="button" onClick={() => { setCountUnit(u); setIsUnitPickerOpen(false); }} className="w-full text-left px-4 py-2.5 text-xs font-bold text-zinc-300 hover:bg-white/10 hover:text-white transition-colors">{u}</button>))}
                                          <div className="h-px bg-white/10 my-1 mx-2" />
                                          <button type="button" onClick={() => setIsCustomUnitModalOpen(true)} className="w-full text-left px-4 py-2.5 text-xs font-bold text-white hover:bg-white/10 transition-colors flex items-center justify-between">Custom Units <ChevronRight size={12} /></button>
                                      </div>
                                  </motion.div>
                              )}
                          </AnimatePresence>
                  ...
                  <div className="flex items-center justify-center gap-2 mt-3">
                       ...
                       <div className="relative">
                          <button type="button" onClick={() => setIsPeriodPickerOpen(!isPeriodPickerOpen)} className="px-3 py-1.5 bg-zinc-800 rounded-[12px] border border-white/5 flex items-center gap-2 active:bg-zinc-700"><span className="text-xs font-bold text-white">{countPeriod}</span><ChevronDown size={10} className="text-zinc-500" /></button>
                          <AnimatePresence>{isPeriodPickerOpen && (<motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} className="absolute top-full mt-2 left-1/2 -translate-x-1/2 min-w-[100px] bg-zinc-900 border border-white/10 rounded-[16px] overflow-hidden shadow-xl z-50 py-1">{PERIODS.map(p => (<button key={p} type="button" onClick={() => { setCountPeriod(p); setIsPeriodPickerOpen(false); }} className="w-full text-center px-4 py-2 text-xs font-bold text-zinc-300 hover:bg-white/10 hover:text-white">{p}</button>))}</motion.div>)}</AnimatePresence>
                       </div>
                  </div>
              </div>
              <div className="w-full h-px bg-white/5 my-6" />
    ...
    <div className="min-h-full flex flex-col justify-end px-6 pb-28 pt-2">
      ...
      <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={() => setHasReminder(!hasReminder)} className={`w-12 h-7 rounded-full transition-colors duration-300 relative ${hasReminder ? 'bg-blue-500' : 'bg-zinc-700'}`}><div className={`absolute top-1 bottom-1 w-5 h-5 bg-white rounded-full shadow-md transition-all duration-300 ${hasReminder ? 'left-6' : 'left-1'}`} /></button>
          </div>
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                             ...
                             <button type="button" onClick={handleAddReminderTime} className="w-14 h-14 bg-zinc-900 border border-white/10 rounded-[20px] flex items-center justify-center text-zinc-400 active:bg-white/5 active:text-white transition-colors"><Plus size={20}/></button>
                        </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {reminderTimes.map(t => (
                                    <div key={t} className="px-3 py-1.5 bg-zinc-800 rounded-[12px] border border-white/5 flex items-center gap-2">
                                        ...
                                        <button type="button" onClick={() => handleRemoveReminderTime(t)} className="text-zinc-500 hover:text-white"><X size={12}/></button>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        <div>
                            <div className="flex gap-1 flex-wrap justify-between mt-2">
                                {WEEKDAYS.map((day, idx) => (
                                    <button key={day} type="button" onClick={() => toggleReminderDay(idx)} className={`w-8 h-8 rounded-full flex items-center justify-center text-[9px] font-black uppercase transition-all ${reminderDays.includes(idx) ? 'bg-white text-black shadow-lg scale-105' : 'bg-zinc-800 text-zinc-500 border border-white/5'}`}>{day.charAt(0)}</button>
                                ))}
                            </div>
                        </div>
                    </div>
          ...
          <>
            ...
            <div className="mb-6">
                ...
                <div className="flex gap-2">
                     ...
                     <button type="button" onClick={addMicroHabit} className="w-12 h-12 bg-zinc-800 border border-white/10 rounded-[16px] flex items-center justify-center text-white active:bg-white/10 transition-colors"><Plus size={18} /></button>
                     {activeParentId && <button type="button" onClick={() => setActiveParentId(null)} className="w-12 h-12 bg-red-500/10 border border-red-500/20 rounded-[16px] flex items-center justify-center text-red-400 active:bg-red-500/20 transition-colors"><X size={18} /></button>}
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
                <button type="button" onClick={() => setIsIncrementModalOpen(true)} className="h-20 bg-zinc-900 border border-white/10 rounded-[24px] flex flex-col items-center justify-center gap-2 active:bg-zinc-800 transition-colors">
                ...
                <button type="button" onClick={() => setIsTimerSettingsModalOpen(true)} className="h-20 bg-zinc-900 border border-white/10 rounded-[24px] flex flex-col items-center justify-center gap-2 active:bg-zinc-800 transition-colors">
    ...
    <div className="w-full flex flex-col h-[65vh] relative">
       ...
       <div className="absolute bottom-0 left-0 right-0 z-40 px-6 pb-6 pt-10 bg-gradient-to-t from-black/80 to-transparent pointer-events-none">
        <div className="flex gap-4 pointer-events-auto">
          {!(step === 1 && initialData) && (
            <button type="button" onClick={step === 1 ? onBack : () => setStep(1)} className="flex-1 py-5 bg-zinc-900 border border-white/10 rounded-[20px] text-[10px] font-black uppercase tracking-[0.2em] text-white active:bg-white/10 transition-colors flex items-center justify-center gap-2">{step === 1 ? <><ChevronLeft size={14} /> Back</> : 'Cancel'}</button>
          )}
          <button type="button" onClick={step === 1 ? handleNextStep : handleFinalSave} className={`flex-1 py-5 bg-white text-black rounded-[20px] text-[10px] font-black uppercase tracking-[0.2em] active:scale-95 transition-all shadow-xl shadow-white/5 flex items-center justify-center gap-2`}>{step === 1 ? <><span className="mr-1">Next</span> <ChevronRight size={14} /></> : 'Save Habit'}</button>
        </div>
      </div>
         ...
         {isCustomUnitModalOpen && (
             <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-md flex flex-col items-center justify-end sm:justify-center" onClick={(e) => e.stopPropagation()}>
                 <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="w-full bg-[#121212] rounded-t-[32px] sm:rounded-[32px] p-6 pb-12 border-t sm:border border-white/10 shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex items-center justify-between mb-8"><h2 className="text-xl font-black uppercase tracking-tighter text-white">Custom Units</h2><button type="button" onClick={() => setIsCustomUnitModalOpen(false)} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-zinc-400"><X size={16} /></button></div>
                    ...
                    <div className="flex gap-3"><button type="button" onClick={() => setIsCustomUnitModalOpen(false)} className="flex-1 h-14 rounded-[20px] bg-zinc-900 border border-white/10 text-[10px] font-black uppercase tracking-widest text-zinc-400">Cancel</button><button type="button" onClick={handleSaveCustomUnit} className="flex-1 h-14 rounded-[20px] bg-white text-black text-[10px] font-black uppercase tracking-widest shadow-lg">Save Unit</button></div>
                 </motion.div>
             </motion.div>
         )}
      </AnimatePresence>
                 ...
                 <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="w-full bg-[#121212] rounded-t-[32px] sm:rounded-[32px] p-6 pb-12 border-t sm:border border-white/10 shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="flex items-center justify-center gap-4 mb-6">
                        <div className="h-16 bg-zinc-900 border border-white/10 rounded-[24px] px-3 flex items-center gap-4">
                            <button type="button" onClick={() => setIncrementAmount(Math.max(1, Number(incrementAmount) - 1))} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-zinc-400"><Minus size={18}/></button>
                            ...
                            <button type="button" onClick={() => setIncrementAmount(Number(incrementAmount) + 1)} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-zinc-400"><Plus size={18}/></button>
                        </div>
                    ...
                    <div className="flex gap-3"><button type="button" onClick={() => setIsIncrementModalOpen(false)} className="flex-1 h-14 rounded-[20px] bg-zinc-900 border border-white/10 text-[10px] font-black uppercase tracking-widest text-zinc-400">Close</button><button type="button" onClick={() => setIsIncrementModalOpen(false)} className="flex-1 h-14 rounded-[20px] bg-white text-black text-[10px] font-black uppercase tracking-widest shadow-lg">Apply</button></div>
                 </motion.div>
             </motion.div>
         )}
      </AnimatePresence>
                    ...
                    <div className="mb-6 space-y-6 px-2">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex bg-zinc-900 rounded-[14px] p-1 border border-white/5">
                                <button type="button" onClick={() => setTimerDefault('stopwatch')} className={`px-3 py-2 rounded-[10px] text-[10px] font-bold uppercase transition-all ${timerDefault === 'stopwatch' ? 'bg-white text-black shadow-md' : 'text-zinc-500'}`}>Stopwatch</button>
                                <button type="button" onClick={() => setTimerDefault('countdown')} className={`px-3 py-2 rounded-[10px] text-[10px] font-bold uppercase transition-all ${timerDefault === 'countdown' ? 'bg-white text-black shadow-md' : 'text-zinc-500'}`}>Countdown</button>
                            </div>
                        </div>
                        {timerDefault === 'countdown' && (
                             <div className="flex items-center justify-between">
                                ...
                                <div className="bg-zinc-900 border border-white/10 rounded-[16px] px-4 py-2 flex items-center gap-2">
                                     <button type="button" onClick={() => setCountdownDuration(Math.max(1, countdownDuration - 5))} className="text-zinc-500 active:text-white"><Minus size={14}/></button>
                                     <span className="text-sm font-black text-white w-12 text-center">{countdownDuration} m</span>
                                     <button type="button" onClick={() => setCountdownDuration(countdownDuration + 5)} className="text-zinc-500 active:text-white"><Plus size={14}/></button>
                                </div>
                             </div>
                        )}
                    </div>
                    <div className="flex gap-3 mt-8"><button type="button" onClick={() => setIsTimerSettingsModalOpen(false)} className="flex-1 h-14 rounded-[20px] bg-zinc-900 border border-white/10 text-[10px] font-black uppercase tracking-widest text-zinc-400">Cancel</button><button type="button" onClick={() => setIsTimerSettingsModalOpen(false)} className="flex-1 h-14 rounded-[20px] bg-white text-black text-[10px] font-black uppercase tracking-widest shadow-lg">Apply</button></div>
                 </motion.div>
             </motion.div>
         )}
      </AnimatePresence>
