                        ...
                        return (
                            <g key={i} className="group cursor-pointer">
                                ...
                                <g className="opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                                    <rect x={x - 30} y={y - 45} width="60" height="34" rx="10" fill="#0a0a0a" stroke="rgba(255,255,255,0.2)" strokeWidth="1" />
                                    <text x={x} y={y - 33} textAnchor="middle" fill="rgba(255,255,255,0.4)" fontSize="7" fontWeight="black" className="uppercase tracking-tighter">
                                        {new Date(d.date).toLocaleDateString('it-IT', { day: '2-digit', month: 'short' })}
                                    </text>
                                    <text x={x} y={y - 20} textAnchor="middle" fill="white" fontSize="11" fontWeight="black">
                                        {d.value} <tspan fontSize="7" fill="rgba(255,255,255,0.3)">{unit}</tspan>
                                    </text>
                                </g>
                            </g>
                        );
...
const getHabitConfig = (habit: Habit) => {
    let config = { type: 'every', days: [] as number[], daysPerWeek: 7 };
    try {
...
const getWeeklyCompletions = (habit: Habit, currentDate: Date) => {
    ...
    for(let i=0; i<7; i++) {
        ...
        if (config.type === 'specific') {
            const formDay = (d.getDay() + 6) % 7;
            if (!config.days.includes(formDay)) continue;
        }
...
const HabitCard: React.FC<HabitCardProps> = ({ habit, onClick, onQuickAdd, onSkip, onTimer, onDeleteRequest, onEditRequest, currentDate, dayEndTime }) => {
  ...
  const isFlexible = config.type === 'days_per' || config.type === 'specific'; 
  const weeklyTarget = config.type === 'specific' ? config.days.length : (config.type === 'days_per' ? config.daysPerWeek : 7);
  ...
  const weekData = useMemo(() => {
    ...
    const startOfWeek = new Date(dStart.setDate(diff));
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const isScheduled = config.type === 'every' || 
                           (config.type === 'specific' && config.days.includes(formDay)) ||
                           (config.type === 'days_per'); 
        ...
        else if (k < adjustedTodayKey && isScheduled) { status = 'failed'; } 
        
        days.push({ 
            dayName: d.toLocaleDateString('en-US', { weekday: 'narrow' }), 
            status, 
            isToday: k === adjustedTodayKey,
            isScheduled 
        });
    }
    return days;
  }, [habit.history, currentDate, habit.goal, dayEndTime, habit, config]);

  const getDynamicStatusClasses = (status: string, isToday: boolean, isScheduled: boolean) => {
      ...
      switch (status) {
        ...
        case 'empty': default: 
            if (!isScheduled && config.type === 'specific') return 'bg-white/[0.01] border-transparent text-zinc-800 opacity-20';
            return isToday ? 'bg-transparent border-white text-white' : 'bg-white/5 border-transparent text-zinc-600';
    }
  };
  ...
  const formDay = (jsDay + 6) % 7;
  const canLogToday = config.type !== 'specific' || config.days.includes(formDay);

  return (
    <div className={`relative w-full mb-3 last:mb-0 transition-opacity ${!canLogToday ? 'opacity-50' : 'opacity-100'}`}>
            ...
            <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button disabled={!canLogToday} onClick={(e) => { e.stopPropagation(); hasStructure ? onClick() : onQuickAdd(e); }} className="relative w-10 h-10 flex items-center justify-center active:scale-90 transition-transform disabled:opacity-20">
...
const HabitsReportOverlay: React.FC<{ habits: Habit[]; onClose: () => void; dayEndTime: string; }> = ({ habits, onClose, dayEndTime }) => {
    ...
    const getRangeLabel = () => { 
        if (timeRange === 'week') { 
            ...
            return `${start.getDate()} ${start.toLocaleString('default', { month: 'short' }).toUpperCase()} - ${end.getDate()} ${end.toLocaleString('default', { month: 'short' }).toUpperCase()}`; 
        } 
        if (timeRange === 'month') { 
            ...
            <div className="px-6 mb-8 flex-shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <div className="bg-zinc-900 rounded-[24px] p-1 flex items-center border border-white/5 h-14">
                        ...
                        <button onClick={() => setViewDate(new Date())} className="px-5 h-10 rounded-[18px] bg-white text-black font-black text-[10px] uppercase tracking-widest active:scale-95 transition-transform flex items-center shadow-lg">Today</button>
...
// MODALS
const QuickTimerModal: React.FC<{ habit: Habit; onClose: () => void; onSave: (val: number) => void; }> = ({ habit, onClose, onSave }) => {
    const styles = getColorClasses(habit.color);
...
const WeightLogModal: React.FC<{ habit: Habit, onClose: () => void, onSave: (val: number) => void, currentDate: Date }> = ({ habit, onClose, onSave, currentDate }) => {
    const dateKey = getLocalDateKey(currentDate); const val = habit.history[dateKey]; const [weight, setWeight] = useState(val && typeof val === 'number' ? String(val) : '');
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[130] bg-black/80 backdrop-blur-md flex items-center justify-center p-6" onClick={onClose}><motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-sm bg-[#121212] border border-white/10 rounded-[36px] p-6 shadow-2xl overflow-hidden relative"><div className="flex justify-between items-center mb-6"><div className="flex items-center gap-3"><span className="text-lg font-black uppercase tracking-tighter text-white">{habit.name}</span></div><button onClick={onClose} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-zinc-400 active:bg-white/10"><X size={18} /></button></div><div className="flex flex-col items-center justify-center w-full min-h-0 mb-8 pt-4"><div className="flex items-baseline gap-2"><input type="number" step="0.1" autoFocus value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="0.0" className="bg-transparent text-right text-6xl font-black text-white focus:outline-none w-40 placeholder:text-zinc-800" /><span className="text-xl font-bold text-zinc-500 uppercase">{habit.unit}</span></div><p className="text-zinc-600 text-[10px] font-bold uppercase tracking-widest mt-2">Log for {currentDate.toLocaleDateString()}</p></div><div className="flex gap-3"><button onClick={onClose} className="flex-1 h-12 rounded-[20px] bg-zinc-900 border border-white/10 text-[10px] font-black uppercase tracking-widest text-zinc-400">Cancel</button><button onClick={() => { const v = parseFloat(weight); if (!isNaN(v)) onSave(v); }} className="flex-1 h-12 rounded-[20px] bg-white text-black text-[10px] font-black uppercase tracking-widest shadow-lg">Save</button></div></motion.div></motion.div>);
};

const SkipConfirmModal = ({ habit, onConfirm, onCancel }: { habit: Habit, onConfirm: () => void, onCancel: () => void }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6" onClick={onCancel}>
    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="bg-[#121212] rounded-[32px] p-6 w-full max-w-xs border border-white/10 shadow-2xl text-center">
      <div className="w-16 h-16 rounded-full bg-zinc-800 mx-auto flex items-center justify-center mb-4 text-orange-400 border border-white/5"><AlertCircle size={32} /></div>
      <h3 className="text-xl font-bold mb-2 text-white">Salta Abitudine</h3>
      <p className="text-zinc-500 text-xs mb-6 px-2 leading-relaxed">Vuoi davvero saltare <span className="text-white font-bold">"{habit.name}"</span> per oggi?</p>
      <div className="flex w-full gap-3">
        ...
        <button onClick={onConfirm} className="flex-1 py-4 rounded-[20px] bg-orange-500 font-bold text-white shadow-lg active:scale-95 transition-transform text-xs uppercase tracking-widest">Salta</button>
      </div>
    </motion.div>
  </motion.div>
);

const DeleteConfirmModal = ({ habit, onConfirm, onCancel }: { habit: Habit, onConfirm: () => void, onCancel: () => void }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6" onClick={onCancel}>
    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} onClick={(e) => e.stopPropagation()} className="bg-[#121212] rounded-[32px] p-6 w-full max-w-xs border border-white/10 shadow-2xl text-center">
      <div className="w-16 h-16 rounded-full bg-zinc-800 mx-auto flex items-center justify-center mb-4 text-red-500 border border-white/5"><Trash2 size={32} /></div>
      <h3 className="text-xl font-bold mb-2 text-white">Elimina Abitudine</h3>
      <p className="text-zinc-500 text-xs mb-6 px-2 leading-relaxed">Vuoi davvero eliminare <span className="text-white font-bold">"{habit.name}"</span>? Questa azione è irreversibile.</p>
      <div className="flex w-full gap-3">
        <button onClick={onCancel} className="flex-1 py-4 rounded-[20px] bg-white/5 font-bold text-zinc-400 active:bg-white/10 text-xs uppercase tracking-widest">Annulla</button>
        <button onClick={onConfirm} className="flex-1 py-4 rounded-[20px] bg-red-600 font-bold text-white shadow-lg active:scale-95 transition-transform text-xs uppercase tracking-widest">Elimina</button>
      </div>
    </motion.div>
  </motion.div>
);
...
const HabitDetailOverlay: React.FC<HabitDetailOverlayProps> = ({ habit, onClose, state, setState, currentDate, initialTab = 'check', onAddHabit, dayEndTime }) => {
  ...
  const lastRecordedWeight = isWeight ? (historyEntries.length > 0 ? historyEntries[historyEntries.length - 1].value : 0) : 0;
  ...
  useEffect(() => { setTimerRunning(false); if(timerMode === 'stopwatch') setTimerSeconds(0); else { const d = habit.unit === 'minutes' ? habit.goal*60 : 15*60; setInitialCountdown(d); setTimerSeconds(d); } }, [timerMode, habit.goal, habit.unit, activeTab, initialTab]); 
  useEffect(() => { if(timerRunning) { intervalRef.current = window.setInterval(() => { if(timerMode === 'stopwatch') setTimerSeconds(s => s + 1); else setTimerSeconds(s => { if(s<=1){ setTimerRunning(false); return 0;} return s-1; }); }, 1000); } else if (intervalRef.current) window.clearInterval(intervalRef.current); return () => { if(intervalRef.current) window.clearInterval(intervalRef.current); }; }, [timerRunning, timerMode]);
  ...
  const toggleMicroHabit = (microId: string) => {
      ...
      if (entry && typeof entry === 'object' && 'completedIds' in entry) currentCompletedIds = [...entry.completedIds];
      if (currentCompletedIds.includes(microId)) currentCompletedIds = currentCompletedIds.filter(id => id !== microId);
      else currentCompletedIds.push(microId);
  ...
  const renderChecklist = (items: MicroHabit[], level = 0) => {
      const entry = habit.history[dateKey];
      let completedIds: string[] = [];
      if (entry && typeof entry === 'object' && 'completedIds' in entry) completedIds = entry.completedIds;
              ...
              {items.map(item => {
                  ...
                  return (
                      <div key={item.id} className="flex flex-col">
                          <button onClick={() => toggleMicroHabit(item.id)} className={`flex items-center gap-3 p-3 rounded-2xl border transition-all ${isChecked ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-white/5 border-white/5 active:bg-white/10'}`}>
  ...
  const now = new Date();
  const [endH, endM] = (dayEndTime || "00:00").split(':').map(Number);
  if (now.getHours() < endH || (now.getHours() === endH && now.getMinutes() < endM)) now.setDate(now.getDate() - 1);
  const adjustedTodayKey = getLocalDateKey(now);
  ...
  return (
    <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-6">
        ...
        <motion.div layoutId={`habit-card-${habit.id}`} className="w-full max-w-sm h-[85vh] bg-[#0a0a0a] overflow-hidden relative shadow-2xl border border-white/10 flex flex-col z-10" onClick={(e) => e.stopPropagation()}>
             <div className="w-full h-full flex flex-col relative">
                                ...
                                <div className="flex flex-col items-center flex-shrink-0 px-6 w-full">
                                    ...
                                    {!isWeight && (
                                        <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full backdrop-blur-md border border-white/5 shadow-lg ${isWeeklyTargetMet ? 'bg-emerald-500/20 border-emerald-500/30' : 'bg-white/10'}`}>
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-widest text-white">{habit.streak} {isFlexible ? 'Wks' : 'Days'}</span>
                                        </div>
                                    )}
                                </div>
                                {hasStructure ? (
                                    <div className="flex-1 w-full flex flex-col min-h-0 py-4 px-6 overflow-hidden">
                                        ...
                                        <div className="w-full h-1 bg-zinc-800 rounded-full overflow-hidden mb-6">
                                            <motion.div initial={{ width: 0 }} animate={{ width: `${structPercentage}%` }} className={`h-full ${structPercentage >= 100 ? 'bg-emerald-500' : 'bg-blue-500'}`} />
                                        </div>
                                        <div className="flex-1 overflow-y-auto no-scrollbar pb-10">{renderChecklist(dailyStructure)}</div>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        <div className="relative w-56 h-56 flex-shrink-0 flex items-center justify-center">
                                                {!isWeight && (
                                                    <svg className="w-full h-full rotate-[-90deg] drop-shadow-2xl">
                                                        ...
                                                        <motion.circle initial={{ pathLength: 0 }} animate={{ pathLength: currentValue === -1 ? 0 : Math.min(1, currentValue / habit.goal) }} cx="50%" cy="50%" r="45%" className={`stroke-${(isFlexible && !isWeeklyTargetMet) ? habit.color : 'emerald'}-500`} strokeWidth="20" fill="none" strokeLinecap="round" />
                                                    </svg>
                                                )}
                                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                    <div className="flex flex-col items-center">
                                                        ...
                                                        <div className="flex flex-col items-center mt-2">
                                                            ...
                                                            <span className="text-sm font-black text-zinc-300">{isWeight ? (habit.goal > 0 && <span>{habit.goal} {habit.unit}</span>) : <>{habit.goal} <span className="text-[9px] opacity-60">{habit.unit}</span></>}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                        </div>
                                    ...
                                    {isWeight ? (
                                        <>
                                            <div className="flex-shrink-0 flex flex-col items-center mb-6">
                                                ...
                                                <div className="flex items-baseline gap-1"><span className="text-4xl font-black text-white">{habit.goal > 0 ? habit.goal : lastRecordedWeight}</span><span className="text-xs font-bold text-zinc-500">{habit.unit}</span></div>
                                            </div>
                                            <div className="w-full flex-shrink-0 mb-6 relative" style={{ height: '300px' }}><SimpleLineChart data={historyEntries} color={habit.color} unit={habit.unit} /></div>
                                            <div className="space-y-2 w-full">{historyEntries.slice().reverse().map((entry, idx) => (<div key={idx} className="flex items-center justify-between p-4 bg-zinc-900/50 rounded-[20px] border border-white/5"><span className="text-xs font-bold text-zinc-400">{new Date(entry.date).toLocaleDateString()}</span><div className="flex items-center gap-1"><span className="text-sm font-black text-white">{entry.value}</span><span className="text-[10px] font-bold text-zinc-600">{habit.unit}</span></div></div>))}</div>
                                        </>
                                    ) : (
                                        <>
                                            <h2 className="text-2xl font-black uppercase tracking-tighter mb-6 text-white text-center">Calendar</h2>
                                            <div className="bg-zinc-900/50 rounded-[24px] overflow-hidden w-full border border-white/5">
                                                <div className="grid grid-cols-7 gap-[1px] bg-zinc-900/50">
                                                    {Array.from({length: 35}).map((_, i) => {
                                                        const today = new Date(); const d = new Date(today.getFullYear(), today.getMonth(), 1); const startDay = d.getDay() === 0 ? 6 : d.getDay() - 1; 
                                                        const dayNum = i - startDay + 1; const isValid = dayNum > 0 && dayNum <= 31; 
                                                        if (!isValid) return <div key={i} className="aspect-square bg-transparent" />;
                                                        const actualDate = new Date(today.getFullYear(), today.getMonth(), dayNum); const k = getLocalDateKey(actualDate); const valEntry = habit.history[k];
                                                        let isDone = false; let isSkipped = false; let isPartial = false;
                                                        if (typeof valEntry === 'number') { isDone = valEntry >= habit.goal; isSkipped = valEntry === -1; isPartial = valEntry > 0 && valEntry < habit.goal; }
                                                        else if (valEntry && typeof valEntry === 'object') { const s = getDailyStructure(habit, actualDate); if (s.length) { const p = getStructureProgress(habit, k, s).percentage; isDone = p >= 100; isPartial = p > 0 && !isDone; } }
                                                        const isFuture = dayNum > today.getDate(); const weeklyCompletions = getWeeklyCompletions(habit, actualDate); const isWeeklyMet = weeklyCompletions >= weeklyTarget; const useGreen = !isFlexible || isWeeklyMet; const isPast = k < adjustedTodayKey;
                                                        let cellClass = 'bg-zinc-800/80 text-zinc-500'; if (isFuture) cellClass = 'bg-zinc-900/30 text-zinc-700'; else if (isSkipped) cellClass = 'bg-orange-500/20 text-orange-400'; else if (isPartial) cellClass = `bg-${habit.color}-500/20 text-${habit.color}-400`; else if (isDone) cellClass = useGreen ? 'bg-emerald-500/30 text-emerald-400' : `bg-${habit.color}-500/30 text-${habit.color}-400`; else if (isPast && !isDone) cellClass = 'bg-red-500/20 text-red-400';
                ...
                <div className="absolute bottom-6 left-0 right-0 px-6 z-40 flex justify-center">
                    <div className="p-2 rounded-[32px] flex w-full max-w-sm bg-zinc-900/80 backdrop-blur-md border border-white/10 shadow-2xl">
                        {[ { id: 'entries', label: isWeight ? 'Report' : 'Calendar', icon: isWeight ? TrendingUp : Calendar }, { id: 'check', label: 'Check', icon: CheckSquare }, !isWeight ? { id: 'timer', label: 'Timer', icon: Clock } : null ].filter(Boolean).map((tab: any) => (<button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex-1 py-4 rounded-[24px] flex flex-col items-center justify-center gap-1.5 transition-all duration-300 ${activeTab === tab.id ? 'bg-white text-black shadow-lg scale-100' : 'text-zinc-500 hover:bg-white/5 scale-95'}`}><tab.icon size={20} /><span className="text-[9px] font-black uppercase tracking-widest">{tab.label}</span></button>))}
                    </div>
                </div>
             </div>
        </motion.div>
...
const HabitsView: React.FC<HabitsViewProps> = ({ state, setState, onDetailOpen, onAddHabit, onEditHabit }) => {
  ...
  const updateHabitValue = (habit: Habit, delta?: number) => {
    ...
    const jsDay = currentDate.getDay();
    const formDay = (jsDay + 6) % 7;
    if (config.type === 'specific' && !config.days.includes(formDay)) return;

    let currentVal = 0;
    if (typeof entry === 'number') currentVal = entry === -1 ? 0 : entry;
    
    const increment = delta !== undefined ? delta : (habit.increment || 1);
    ...
    <div className="flex flex-col h-full w-full">
        <header className="flex-shrink-0 px-6 pt-16 pb-6">
            ...
            <div className="flex items-center justify-between bg-zinc-900/50 rounded-[20px] p-1 border border-white/5">
                ...
                <div className="flex flex-col items-center">
                    <span className="text-sm font-black uppercase tracking-widest text-white">{currentDate.toLocaleDateString('en-US', { weekday: 'short', day: 'numeric', month: 'short' })}</span>
             ...
             <div className="space-y-4">
                 {sortedHabits.length > 0 ? sortedHabits.map(habit => (
                     <HabitCard 
                        ...
                        onQuickAdd={(e) => {
                             if (habit.unit === 'kg' || habit.unit === 'lbs') setWeightLogHabit(habit);
                             else updateHabitValue(habit);
                        }}
