...
const HabitsReportOverlay: React.FC<HabitReportOverlayProps> = ({ habits, onClose, dayEndTime }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="fixed inset-0 z-[100] bg-black/95 flex flex-col">
             <div className="px-6 pt-[calc(env(safe-area-inset-top)+1.5rem)] flex items-center justify-between mb-6 flex-shrink-0">
             ...
             <div className="w-full h-full flex flex-col relative">
                ...
                <div className="flex-1 flex flex-col relative overflow-hidden z-10">
                    <AnimatePresence mode="wait">
                         ...
                         {activeTab === 'entries' && (
                             <motion.div key="entries" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="flex-1 w-full flex flex-col relative overflow-hidden">
...
const HabitsView: React.FC<Props> = ({ state, setState, onDetailOpen, onAddHabit, onEditHabit }) => {
  ...
  return (
    <div className="flex flex-col h-full w-full">
        <header className="flex-shrink-0 px-6 pt-[calc(env(safe-area-inset-top)+1.5rem)] pb-6">
